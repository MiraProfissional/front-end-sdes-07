...
describe('<Footer />', () => {
  ...
  it('should render texts about Contato', () => {
    ...
    expect(screen.getByText('Rua Araguaia, 70 - Vila São Bento São José dos Campos - São Paulo')).toBeInTheDocument();
  ...
  it('should have the corrects atributes: Redes Sociais', () => {
    ...
    expect(screen.getByText('Facebook')).toHaveAttribute('href','https://pt-br.facebook.com/trakmaq/')
    expect(screen.getByText('Facebook')).toHaveAttribute('target','_blank')
    expect(screen.getByText('Instagram')).toHaveAttribute('href','https://www.instagram.com/trakmaq/')
    expect(screen.getByText('Instagram')).toHaveAttribute('target','_blank')
    expect(screen.getByText('Whatsapp')).toHaveAttribute('href','https://api.whatsapp.com/send?phone=5512997833901')
    expect(screen.getByText('Instagram')).toHaveAttribute('target','_blank')
  })
